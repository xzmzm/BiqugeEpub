...
class BiqugeEpub(object):
    ...
    def generate_epub(self):
        try:
            ...
            #print "book_info['authorurl'] is", book_info.get('authorurl')
            ...
            if len(title_all)>9: title_all=title_all[9:]
            ...
            # catalog.html
            cat_t=file('catalog.html','r+')
            cat_str=cat_t.read()
            cat_str=cat_str.replace('\r',"")
            cat_t.seek(0)
            
            # toc.ncx
            toc_t=file('toc.ncx','r+')
            toc_str=toc_t.read()
            toc_t.seek(0)
            
            # content.opf
            con_t=file('content.opf','r+')
            con_str=con_t.read()
            con_t.seek(0)
            ...
            for title in title_all:
                ...
                w_c=read_file(r'content.html').replace("{{title}}",title[1]).replace("{{content}}",content)
            ...
            #print e
            print "Test whether you can read this novel on <biqugecom.com> with your browser. If it's okay, you can re-run program again or feedback this issue to <https://github.com/hipro/BiqugeEpub.git>."
...
# def test():
#     epub=BiqugeEpub("末世盗贼行")#create a BiqugeEpub class
